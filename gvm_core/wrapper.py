...
class GVMProcessor:
    def __init__(
        self, model_base=None, unet_base=None, lora_base=None, device="cpu", seed=None
    ):
    ...
    def process_sequence(
        ...
        if is_video:
            ...
            image_files = sorted(
                [
                    ...
                    if f.is_file()
                    and f.suffix.lower() in [".jpg", ".png", ".jpeg", ".exr"]
                ]
            )
            if not image_files:
        ...
        transform = Compose(
            [ToTensor(), Resize(size=1024, max_size=max_res_param, antialias=True)]
        )

        if is_video:
            reader = VideoReader(
                str(input_path), max_frames=max_frames, transform=transform
            )
        else:
        ...
        if direct_output_dir:
            ...
            if write_video:
                ...
                writer_alpha = VideoWriter(
                    osp.join(direct_output_dir, f"{file_name}_alpha.mp4"),
                    frame_rate=fps,
                )
        else:
            ...
            writer_alpha = (
                VideoWriter(
                    osp.join(file_output_dir, f"{file_name}_alpha.mp4"), frame_rate=fps
                )
                if write_video
                else None
            )
